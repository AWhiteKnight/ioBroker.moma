...
/**
 * Class Daily implements the logic used in daily interval by moma
 *  (c) 2019 AWhiteKnight
 */
class Daily extends Interval {
    constructor() {
        super();
                            ...
                            adapter.getForeignState(states[j]._id, function(err, state) {
                                if(err) {
                                    ...
                                    if(state.val == true) {
                                        ...
                                        hostNeedsRebootList += channels[i]._id + ' ';
                                    }
                                }
                            });
                                ...
                                if(err) {
                                    ...
                                    if(state.val > 0) {
                                        ...
                                        hostNeedsUpdateList += channels[i]._id + ' ';
                                    }
                                }
                            });
                        }
    ...
    */
    ...
    osInfo(adapter) {
        if(adapter.config.osInfo) {
          this.si.osInfo(function(data) {
                this.showData(data, 'osInfo', adapter);
            });
        }
    }
    ...
    uuid(adapter) {
        if(adapter.config.uuid) {
          this.si.uuid(function(data) {
                this.showData(data, 'uuid', adapter);
            });
        }
    }
    ...
    shell(adapter) {
        if(adapter.config.shell) {
            this.si.shell(function(data) {
                this.showData(data, 'shell', adapter);
            });
        }
    }
    ...
    versions(adapter) {
        if(adapter.config.versions) {
          this.si.versions(function(data) {
                this.showData(data, 'versions', adapter);
            });
        }
    }
