...
class Messages {
    ...
    */
			...
			try {
				...
				for (let i = 0; i < obj.length; i++) {
					if(obj[i].length > 0) {
						...
						switch(os.platform()) {
							case 'linux':
								cmd.execSync('iobroker upgrade ' + obj[i], {cwd:'/opt/iobroker'});
								break;
							case 'win32':
								cmd.execSync('iobroker upgrade ' + obj[i], {cwd:'C:\\iobroker'});
								break;
							case 'darwin':
							default:
	...
	async updateJsController(adapter) {
		...
		switch(os.platform()) {
			case 'linux':
				cmd.spawn('iobroker stop && iobroker upgrade self && iobroker start', {cwd:'/opt/iobroker'});
				//cmd.execSync('iobroker update', {cwd:'/opt/iobroker'});
				//cmd.execSync('iobroker upgrade self', {cwd:'/opt/iobroker'});
				//cmd.execSync('iobroker restart', {cwd:'/opt/iobroker'});
				break;
			case 'win32':
				cmd.spawn('iobroker stop && iobroker upgrade self && iobroker start', {cwd:'C:\\iobroker'});
				//cmd.execSync('iobroker update', {cwd:'C:\\iobroker'});
				//cmd.execSync('iobroker upgrade self', {cwd:'C:\\iobroker'});
				//cmd.execSync('iobroker restart', {cwd:'C:\\iobroker'});
				break;
			case 'darwin':
			default:
