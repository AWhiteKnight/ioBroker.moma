	...
	async updateAdapter(adapter) {
		...
		adapter.getForeignState(defs.hostEntryListOfAdapterUpdates, async (err, state) => {
			...
			try {
				...
				// try to repair
				try {
					// restart admin if necessary
					switch(os.platform()) {
						case 'linux':
						case 'darwin':
							options.cwd = '/opt/iobroker/';
							command = 'iobroker start admin.0';
							break;
						case 'win32':
							options.cwd ='C:\\iobroker';
							command = 'C:\\iobroker\\iob start admin.0 ';
							break;
						default:
							adapter.log.info('update adapter not implemented for ' + os.platform());
							// bail out
				...
				for(let i = 0; i < lines.length; i++) {
					...
					if(lines[i].search('Updateable') > 0) {
						...
						if(adapterName == 'js-controller') {
							...
							switch(os.platform()) {
								case 'win32':
									command2 = `npm install iobroker.js-controller@${newVersion} --ignore-scripts "${options.cwd}"`;
									break;
								default:
