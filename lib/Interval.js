    ...
    static showData(src, tgtId, adapter, init) {
        ...
        // array
            ...
            if(tgtId == 'processes.list') {
                adapter.setStateChanged(tgtId, {val: Interval.createHTMLFromProcessList(src), ack: true});
            } else if(tgtId == 'networkConnections') {
                adapter.setStateChanged(tgtId, {val: Interval.createHTMLFromNetworkConnectionsList(src), ack: true});
            } else {
                let helper;
                if(init) {
