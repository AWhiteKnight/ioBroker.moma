...
class Interval4 extends Interval {
    ...
    async run(adapter, init) {
        try {
            ...
            await this.osInfo(adapter, init);
            await this.uuid(adapter, init);
            await this.shell(adapter, init);
            await this.versions(adapter, init);
        } catch(err) {
    ...
    */
    doUpdates(adapter) {
        ...
        adapter.setForeignState(defs.hostLastUpdate, {val: Date.now(), ack: true});
    }
