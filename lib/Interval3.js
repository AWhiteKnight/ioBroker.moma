/* jshint -W097 */
/* jshint -W030 */
/* jshint strict:true */
/* jslint node: true */
/* jslint esversion: 6 */
// @ts-nocheck
'use strict';

const Interval = require(__dirname + '/Interval.js');

/**
 * Class Interval3 implements the logic used in interval 3 by moma
 *  (c) 2019 AWhiteKnight
 */
class Interval3 extends Interval {
	constructor() {
        super();
    }

    run(adapter, init) {
        this.graphics(adapter, init);
        this.networkInterfaces(adapter, init);
        this.networkInterfaceDefault(adapter, init);
        this.inetLatency(adapter, init);
        this.dockerContainers(adapter, init);
    }
    
    async graphics(adapter, init) {
        if(adapter.config.graphics) {
            await Interval.getSI().graphics()
                .then(data => Interval.showData(data, 'graphics', adapter, init))
                .catch(error => adapter.log.error(error));
        }
    }

    async networkInterfaces(adapter, init) {
        if(adapter.config.networkInterfaces) {
            await Interval.getSI().networkInterfaces()
                .then(data => Interval.showData(data, 'networkInterfaces', adapter, init))
                .catch(error => adapter.log.error(error));
        }
    }  
      
    async networkInterfaceDefault(adapter, init) {
        if(adapter.config.networkInterfaceDefault) {
            await Interval.getSI().networkInterfaceDefault()
                .then(data => Interval.showData(data, 'networkInterfaceDefault', adapter, init))
                .catch(error => adapter.log.error(error));
        }
    }  
      
    async inetLatency(adapter, init) {
        if(adapter.config.networkInterfaceDefault) {
            await Interval.getSI().inetLatency()
                .then(data => Interval.showData(data, 'inetLatency', adapter, init))
                .catch(error => adapter.log.error(error));
        }
    }  
      
    async dockerContainers(adapter, init) {
        if(adapter.config.dockerContainers) {
            await Interval.getSI().dockerContainers()
                // .then(data => adapter.log.debug(this.createHtmlFromDockerContainers(data)))
                // .then(data => adapter.log.debug('docker: ' + JSON.stringify(data)))
                .then(data => Interval.showData(this.createHtmlFromDockerContainers(data), 'dockerContainers', adapter, init))
                .catch(error => adapter.log.error(error));
        }
    }
    
    /**
     * convert object dockerContainers into html table
     * format of dockerContainers:
     * 'dockerContainers': [{
     *          'id',
     *          'name',
     *          'image',
     *          'imageID',
     *          'command',
     *          'created',
     *          'started',
     *          'finished',
     *          'createdAt',
     *          'startedAt',
     *          'finishedAt',
     *          'status',
     *          'ports',
     *          'mounts'
     *      }]
    */
   createHtmlFromDockerContainers(src) {
    let tgt = '<table><thead><tr><th>id</th><th>name</th><th>image</th><th>imageID</th><th>command</th><th>created</th><th>started</th><th>finished</th><th>createdAt</th><th>startedAt</th><th>finishedAt</th><th>state</th><th>ports</th><th>mounts</th></tr></thead><tbody>';
    let el;
    for(const i in src) {
        el = src[i];
        tgt += '<tr>';
            tgt += '<td>' + el.id + '</td>';
            tgt += '<td>' + el.name + '</td>';
            tgt += '<td>' + el.image + '</td>';
            tgt += '<td>' + el.imageID + '</td>';
            tgt += '<td>' + el.command + '</td>';
            tgt += '<td>' + el.created + '</td>';
            tgt += '<td>' + el.started + '</td>';
            tgt += '<td>' + el.finished + '</td>';
            tgt += '<td>' + el.createdAt + '</td>';
            tgt += '<td>' + el.startedAt + '</td>';
            tgt += '<td>' + el.finishedAt + '</td>';
            tgt += '<td>' + el.state + '</td>';
            tgt += '<td>' + el.ports + '</td>';
            tgt += '<td>' + el.mounts + '</td>';
        tgt += '</tr>';
    }
    tgt += '</tbody></table>';
    return tgt;
}
// end of class
}

module.exports = Interval3;