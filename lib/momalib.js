...
module.exports.checkUpdates = (long) => {
  if(adapter.config.updates) {
    maintainStates(getListOfUpdates(long));
  }
}
...
function maintainStates(list) {
  ...
  // TODO maintain global states hostNeedsReboot / hostNeedsUpdate properly regarding other machines
  ...
  switch(machine.platform) {
    case 'linux':
      ...
      if(machine.pkgMgr === "apt") {
        // debian packages with apt 
        cmd.execSync('sudo apt update');
        let result = cmd.execSync('sudo apt full-upgrade -y 2> /dev/null').toString().split('\n');
        ...
        let result = cmd.execSync('sudo yum update -y 2> /dev/null').toString().split('\n');
      } else if(machine.pkgMgr === "zypper") {
        // opennsuse packages with zypper
        cmd.execSync('zypper refresh');
        let result = cmd.execSync('sudo zypper dup -y 2> /dev/null').toString().split('\n');
