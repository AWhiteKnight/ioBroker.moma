...
module.exports.time = (adapter, init) => {
  if(adapter.config.time) {
    let data = si.time();
    if(init) {
      helper.createMomaEntry(adapter, 'time');
      adapter.setState(defs.sysInfo.time.timezone.id, {val: data.timezone, ack: true});
      adapter.setState(defs.sysInfo.time.name.id, {val: data.timezoneName, ack: true});
    }
    adapter.setState(defs.sysInfo.time.current.id, {val: data.current, ack: true});
    adapter.setState(defs.sysInfo.time.uptime.id, {val: data.uptime, ack: true});
  } else {
    if(init) {
...
module.exports.checkUpdates = (adapter) => {
