    ...
    if (defs.sysInfo.hasOwnProperty(template)) {
        ...
        for (const key1 in element) {
            ...
            if(entry1["array"] == "true") {
                ...
                adapter.log.debug('createMomaEntry: nested array in ' + template);
                ...
                for (const key2 in entry1) {
                    ...
                    if(entry2["array"] == "true") {
                        // nested nested array
                        adapter.log.error('createMomaEntry: nested-nested array in ' + template);
                    } else if(!entry2["type"]) {
                        // nested nested object
                        adapter.log.error('createMomaEntry: nested-nested object in ' + template);
                    } else {
                        adapter.log.debug('createMomaEntry: nested object in ' + template);
                        adapter.setObjectNotExists(template+'.'+key1, {
                            type: 'channel',
                            common: {
                                name: key1
...
module.exports.createChannelEntries = (adapter, instances, parentId, type, role) => {
    if(type != 'object') {
        ...
        // object
        adapter.log.debug('array of objects in: ' + parentId);
    }
}
