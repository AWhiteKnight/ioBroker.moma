...
async function createMetaState(adapter, id, varRole, varType) {
	...
	await adapter.setForeignObjectNotExists(state._id, state);
}
...
module.exports.createMomaMetaEntries = (adapter) => {
	...
	createMetaState(adapter, defs.hostNeedsAttention, 'indicator', 'boolean');
	adapter.setForeignState(defs.hostNeedsAttention, {val: false, ack: true});
	createMetaState(adapter, defs.hostNeedsAttentionList, 'text', 'string');

	createMetaState(adapter, defs.hostNeedsUpdate, 'indicator', 'boolean');
	adapter.setForeignState(defs.hostNeedsUpdate, {val: false, ack: true});
	createMetaState(adapter, defs.hostNeedsUpdateList, 'text', 'string');

	createMetaState(adapter, defs.hostNeedsReboot, 'indicator', 'boolean');
	adapter.setForeignState(defs.hostNeedsReboot, {val: false, ack: true});
	createMetaState(adapter, defs.hostNeedsRebootList, 'text', 'string');

	createMetaState(adapter, defs.deviceNeedsBatteryChange, 'indicator', 'boolean');
	adapter.setForeignState(defs.deviceNeedsBatteryChange, {val: false, ack: true});
	createMetaState(adapter, defs.deviceNeedsBatteryChangeList, 'text', 'string');
	...
	createMetaState(adapter, defs.hostEntryInstance, 'text', 'string');

	createMetaState(adapter, defs.hostEntryAlive, 'indicator', 'boolean');
	adapter.setForeignState(defs.hostEntryAlive, {val: false, ack: true, expire: 10000});

	createMetaState(adapter, defs.hostEntryNeedsAttention, 'indicator', 'boolean');
	adapter.setForeignState(defs.hostEntryNeedsAttention, {val: false, ack: true});

	createMetaState(adapter, defs.hostEntryHasUpdates, 'value', 'number');
	createMetaState(adapter, defs.hostEntryListOfUpdates, 'text', 'string');
	createMetaState(adapter, defs.hostEntryLastUpdate, 'date', 'number');

	createMetaState(adapter, defs.hostEntryNeedsReboot, 'indicator', 'boolean');
	adapter.setForeignState(defs.hostEntryNeedsReboot, {val: false, ack: true});

	createMetaState(adapter, defs.hostEntryListOfAdapterUpdates, 'text', 'string');
	createMetaState(adapter, defs.hostEntryControllerUpdate, 'text', 'string');

	// set the instance in moma.meta.<hostname>.instance
	adapter.setForeignStateChanged(defs.hostEntryInstance, {val: adapter.namespace, ack: true});
	adapter.setForeignStateChanged(defs.hostEntryNeedsAttention, {val: false, ack: true});
