...
function removeEntry(adapter, id) {
    adapter.getDevices(function (err, devices) {
        let comp = adapter.namespace + '.' +id;
        let found = false;
        for(let d = 0; d < devices.length; d++) {
            if(devices[d]._id == comp) {
        ...
        if(!found) {
            adapter.getState(id, function (err, state) {
                let sId = adapter.namespace + '.' +id;
                if(state != undefined && state != null) {
                    if(state._id == sId) {
                        adapter.log.info('deleting State ' + state._id);
                        adapter.deleteState(state._id);
                    }
                }
            });
        }
...
module.exports.createMomaMetaEntries = (adapter) => {
    ...
    adapter.setForeignObjectNotExists(device._id, device);
