...
const sysInfo = {
	...
	'uuid': {
		...
		'hardware': {'type': 'string',  'role': 'text'},
		'macs': [
	...
	'currentLoad': {
		...
		'currentLoadNice':       {'type': 'number',  'role': 'value'},
		...
		'rawCurrentLoadSystem':  {'type': 'number',  'role': 'value'},
...
async function releasePreparation(adapter) {
	...
	// cleanup the renamed fields from systeminfo 5.6.7

	const states = await adapter.getStatesOfAsync('currentLoad');
	states.forEach(state => {
		//adapter.log.debug(JSON.stringify(state));
		const name = new String(state.common.name);
		if(name.indexOf('_') > 0) {
			adapter.log.info('deleting: ' + name);
			adapter.delObject(state._id);
		} else if(name == 'currentload') {
			adapter.log.info('deleting: ' + name);
			adapter.delObject(state._id);
		} else if(name == 'avgload') {
			adapter.log.info('deleting: ' + name);
			adapter.delObject(state._id);
		}
	});
...
async function createEntry(adapter, entry, parent, id) {
	const newId = parent+'.'+id;
	if(isStateEntry(entry)) {
		await createStateEntry(adapter, parent, id, entry.type, entry.role);
	} else if(isArrayEntry(entry)) {
		await createChannelEntry(adapter, parent, id);
	} else {
		await createChannelEntry(adapter, parent, id);
		for (const key in entry) {
			...
			if (entry.hasOwnProperty(key)) {
				...
				try {
					await createEntry(adapter, newEntry, newId, key);
				} catch(err) {
...
async function createMomaInstanceEntries(adapter) {
	...
	for (const key0 in sysInfo) {
		...
		if (sysInfo.hasOwnProperty(key0)) {
			...
			// check configuration
			if(adapter.config[key0] || key0 == 'version') {
 ...
 */
module.exports.createArrayEntry = async (adapter, parent, id) => {
	try {
		const comp = parent.split('.');
		if(comp.length == 1)
			await createEntry(adapter, sysInfo[comp[0]][0], parent, id);
		else if(comp.length == 2)
			await createEntry(adapter, sysInfo[comp[0]][comp[1]][0], parent, id);
		else if(comp.length == 3)
			await createEntry(adapter, sysInfo[comp[0]][comp[1]][comp[2]][0], parent, id);
		else {
