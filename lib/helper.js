...
module.exports.releasePreparation = async (adapter) => {
	await adapter.log.debug('release preparation');
	// 1.0.0 -> 1.1.0: delete device networkConnections
	await adapter.getObject('networkConnections', (err, obj) => {
		if(err) {
	...
	await adapter.getObject('cpuFlags', (err, obj) => {
		if(err) {
			adapter.log.error(JSON.stringify(err));
		} else if(obj) {
 ...
 */
module.exports.createMomaInstanceEntries = async (adapter) => {
	await adapter.log.debug('creating moma instance entries');
 ...
 */
module.exports.createMomaMetaEntries = async (adapter) => {
	...
	await adapter.setForeignState(defs.hostNeedsAttention, {val: false, ack: true});
	...
	await adapter.setForeignState(defs.hostNeedsUpdate, {val: false, ack: true});
	...
	await adapter.setForeignState(defs.hostNeedsReboot, {val: false, ack: true});
	...
	await adapter.setForeignState(defs.deviceNeedsBatteryChange, {val: false, ack: true});
	...
	await adapter.setForeignObjectNotExists(channel._id, channel);
	...
	await adapter.setForeignState(defs.hostEntryAlive, {val: false, ack: true, expire: 10000});

	await createMetaState(adapter, defs.hostEntryNeedsAttention, 'indicator', 'boolean');
	await adapter.setForeignState(defs.hostEntryNeedsAttention, {val: false, ack: true});
	...
	await createMetaState(adapter, defs.hostEntryLastUpdate, 'date', 'numbawait er');

	await createMetaState(adapter, defs.hostEntryNeedsReboot, 'indicator', 'boolean');
	await await adapter.setForeignState(defs.hostEntryNeedsReboot, {val: false, ack: true});
	...
	await adapter.setForeignStateChanged(defs.hostEntryInstance, {val: adapter.namespace, ack: true});
	await adapter.setForeignStateChanged(defs.hostEntryNeedsAttention, {val: false, ack: true});
}; 
